...
db_connction = "mysql://{}:{}@{}/kaosdb".format(myAuth.Auth[DB_KEY]["User"], myAuth.Auth[DB_KEY]["Password"], env.DB)
engine = create_engine(db_connction)
...
def SQAGetDataUsageSpecificCPSpecificDate(cradlepoint:str, query_date:str) -> list:
    """
...
def SQAGetDataUsageSpecificCPAll(cradlepoint:str) -> list:
    """
    ...
    stats_schema = CradlepointStatsSchema(many=True)
    return stats_schema.dump(all_usage)

def SQAListAllObjects(output:bool) -> list:
    ...
    stats_schema = CradlepointStatsSchema(many=True)
    #Returns a list of dictionaries each containing {Cradlepoint: "CP-NAME"}
    ...
    cp_list = [list(dict.values())[0] for dict in cp_list_of_dicts]
    if output is True:
...
def SQAFindTopXDeviceUsage(top_x:int=10) -> dict:
...
def SQAFindTopXHighestDays(top_x:int=10) -> dict:
...
def WebSQASystemDailyUsage() -> list:
...
def WebSQALastXTotalUsage(day_limit:int = 0) -> float:
    lastX_days = (
...
def WebSQAAllTimeSystemDataUsage() -> float:
...
def WebSQAOutputObjectInfoByName(cp_name:str) -> dict:
    """
    ...
    max_usage_output = stats_schema.dump(max_usage)
    ...
    earliest_date_output = stats_schema.dump(earliest_date)
    ...
    total_data_used_output = stats_schema.dump(total_data_used)
...
def WebSQAShowUsageByName(cp_name:str, sort_by_value:str, sort_order_value:str, response_limit:int, min_data_usage:str) -> list:
    """
...
def WebSQAAllDeviceUsageForADay(day:str, sort_by_value:str, sort_order_value:str) -> list:
    """
